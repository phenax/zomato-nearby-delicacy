...
/***/ function(module, exports, __webpack_require__) {
	...
	// All components
	(0, _knockout.applyBindings)(new _Root2.default());
...
/***/ function(module, exports, __webpack_require__) {
	...
	if("function"==='function'&&__webpack_require__(3)['amd']){// [1] AMD anonymous module
	...
	var target=ko;for(var i=0;i<tokens.length-1;i++){target=target[tokens[i]];}target[tokens[tokens.length-1]]=object;};ko.exportProperty=function(owner,publicName,object){owner[publicName]=object;};ko.version="3.4.1";ko.exportSymbol('version',ko.version);// For any options that may affect various areas of Knockout and aren't directly associated with data binding.
	...
	if(target.cacheDiffForKnownOperation){return;}var trackingChanges=false,cachedDiff=null,arrayChangeSubscription,pendingNotifications=0,underlyingNotifySubscribersFunction,underlyingBeforeSubscriptionAddFunction=target.beforeSubscriptionAdd,underlyingAfterSubscriptionRemoveFunction=target.afterSubscriptionRemove;// Watch "subscribe" calls, and for array change events, ensure change tracking is enabled
	...
	state.suppressDisposalUntilDisposeWhenReturnsFalse=false;}state.isBeingEvaluated=true;try{changed=this.evaluateImmediate_CallReadWithDependencyDetection(notifyChange);}finally{state.isBeingEvaluated=false;}if(!state.dependenciesCount){computedObservable.dispose();}return changed;},evaluateImmediate_CallReadWithDependencyDetection:function evaluateImmediate_CallReadWithDependencyDetection(notifyChange){// This function is really just part of the evaluateImmediate logic. You would never call it from anywhere else.
	// Factoring it out into a separate function means it can be independent of the try/catch block in evaluateImmediate,
	// which contributes to saving about 40% off the CPU overhead of computed evaluation (on V8 at least).
	var computedObservable=this,state=computedObservable[computedState],changed=false;// Initially, we assume that none of the subscriptions are still being used (i.e., all are candidates for disposal).
	// Then, during evaluation, we cross off any that are in fact still being used.
	var isInitial=state.pure?undefined:!state.dependenciesCount,// If we're evaluating when there are no previous dependencies, it must be the first time
	dependencyDetectionContext={computedObservable:computedObservable,disposalCandidates:state.dependencyTracking,disposalCount:state.dependenciesCount};ko.dependencyDetection.begin({callbackTarget:dependencyDetectionContext,callback:computedBeginDependencyDetectionCallback,computed:computedObservable,isInitial:isInitial});state.dependencyTracking={};state.dependenciesCount=0;var newValue=this.evaluateImmediate_CallReadThenEndDependencyDetection(state,dependencyDetectionContext);if(computedObservable.isDifferent(state.latestValue,newValue)){if(!state.isSleeping){computedObservable["notifySubscribers"](state.latestValue,"beforeChange");}state.latestValue=newValue;if(DEBUG)computedObservable._latestValue=newValue;if(state.isSleeping){computedObservable.updateVersion();}else if(notifyChange){computedObservable["notifySubscribers"](state.latestValue);}changed=true;}if(isInitial){computedObservable["notifySubscribers"](state.latestValue,"awake");}return changed;},evaluateImmediate_CallReadThenEndDependencyDetection:function evaluateImmediate_CallReadThenEndDependencyDetection(state,dependencyDetectionContext){// This function is really part of the evaluateImmediate_CallReadWithDependencyDetection logic.
	...
	ko.bindingContext=function(dataItemOrAccessor,parentContext,dataItemAlias,extendCallback,options){// The binding context object includes static properties for the current, parent, and root view models.
	...
	self._subscribable=subscribable;}else{self['$parents']=[];self['$root']=dataItem;// Export 'ko' in the binding context so it will be available in bindings and templates
	...
	if(extendCallback)extendCallback(self,parentContext,dataItem);return self['$data'];}function disposeWhen(){return nodes&&!ko.utils.anyDomNodeIsAttachedToDocument(nodes);}var self=this,isFunc=typeof dataItemOrAccessor=="function"&&!ko.isObservable(dataItemOrAccessor),nodes,subscribable;if(options&&options['exportDependencies']){// The "exportDependencies" option means that the calling code will track any dependencies and re-create
	// the binding context when they change.
	updateContext();}else{subscribable=ko.dependentObservable(updateContext,null,{disposeWhen:disposeWhen,disposeWhenNodeIsRemoved:true});// At this point, the binding context has been initialized, and the "subscribable" computed observable is
	...
	ko.bindingContext.prototype['createChildContext']=function(dataItemOrAccessor,dataItemAlias,extendCallback,options){return new ko.bindingContext(dataItemOrAccessor,this,dataItemAlias,function(self,parentContext){// Extend the context hierarchy by setting the appropriate pointers
	self['$parentContext']=parentContext;self['$parent']=parentContext['$data'];self['$parents']=(parentContext['$parents']||[]).slice(0);self['$parents'].unshift(self['$parent']);if(extendCallback)extendCallback(self);},options);};// Extend the binding context with new custom properties. This doesn't change the context hierarchy.
	...
	self['$rawData']=parentContext['$rawData'];ko.utils.extend(self,typeof properties=="function"?properties():properties);});};ko.bindingContext.prototype.createStaticChildContext=function(dataItemOrAccessor,dataItemAlias){return this['createChildContext'](dataItemOrAccessor,dataItemAlias,null,{"exportDependencies":true});};// Returns the valueAccesor function for a binding value
	...
	if('if'in options)shouldDisplay=ko.utils.unwrapObservable(options['if']);if(shouldDisplay&&'ifnot'in options)shouldDisplay=!ko.utils.unwrapObservable(options['ifnot']);}if('foreach'in options){// Render once for each data point (treating data set as empty if shouldDisplay==false)
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _ApiHandler = __webpack_require__(6);
	
	var _ApiHandler2 = _interopRequireDefault(_ApiHandler);
	
	var _GoogleMaps = __webpack_require__(11);
	...
	var RootViewModel = function () {
		...
		function RootViewModel() {
			...
			this.loading = _knockout2.default.observable(true);
			this.searchText = _knockout2.default.observable('');
			this.filteredMarkers = _knockout2.default.observableArray();
			this.map = new _GoogleMaps2.default();
			this.markers = new _Markers.Markers(this.map);
			this.util = _Utils2.default;
			...
			.then(function () {
				...
				_Utils2.default.showError(e.message);
			});
		}
		...
		_createClass(RootViewModel, [{
			...
			value: function _initializeMap(coord) {
				...
				// Zomato api wrapper
				var zomato = new _ApiHandler2.default();
	
				// The DOM Node to display the map in
				var $hook = document.getElementById('fendMap');
	
				// Not necessary, just a precaution
				if ($hook === null) _Utils2.default.showError('An Error occured. Try reloading the page.');
			...
			key: '_renderRestaurants',
			value: function _renderRestaurants(data) {
				var _this2 = this;
	...
	var Utils = function () {
		function Utils() {
			...
			this.messageBox = {
				...
				text: _knockout2.default.observable(''),
	
				type: _knockout2.default.observable(''),
	
				visible: _knockout2.default.observable(false)
			};
		...
		_createClass(Utils, [{
			...
			value: function isInfo() {
				return this.messageBox.type() === 'info';
			}
		}]);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var ApiHandler = function () {
		...
		_createClass(ApiHandler, [{
			...
			value: function getUrl(lat, lng, radius) {
	   ...
	   */
			...
			key: 'send',
			value: function send(coords, radius) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(9), __webpack_require__(8), (function() { return this; }())))
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _mapStyles = __webpack_require__(12);
	...
	var GoogleMaps = function () {
		...
		function GoogleMaps() {
			...
			this.API_KEY = 'AIzaSyAjPs0W0u7uHmMEHhJGwZyhTVlEE_5KKso';
			this.API_URL = 'https://maps.googleapis.com/maps/api/js?key=';
			this.STYLES = _mapStyles2.default;
			this.markers = [];
			this._onLoadStack = [];
			this._onErrorStack = [];
	   ...
	   */
			value: function loadMapScript() {
				...
				$script.onload = function () {
					_this._popCallbackStack(_this._onLoadStack);
					_this.loaded = true;
				};
	
				$script.onerror = function () {
					_this._popCallbackStack(_this._onErrorStack);
					_this.error = true;
				};
	   ...
	   */
			...
			key: '_popCallbackStack',
			value: function _popCallbackStack(stack) {
			...
			value: function ready() {
				...
				return new Promise(function (resolve, reject) {
					...
					if (_this2.loaded) {
						...
						_this2._onErrorStack.push(function () {
							reject(error);
						});
	
						// Else, register it to the onLoad stack
						_this2._onLoadStack.push(function () {
							resolve();
						});
					}
				});
	   ...
	   */
			...
			key: 'createMap',
			value: function createMap(center, $hook, zoom) {
...
/***/ function(module, exports) {
	...
	var Markers = exports.Markers = function () {
	   ...
	   */
			...
			key: 'addMarkerWindow',
			value: function addMarkerWindow(data, marker) {
				var _this = this;
	   ...
	   */
			...
			key: '_displayWindow',
			value: function _displayWindow(marker) {
	   ...
	   */
			...
			key: 'showWindow',
			value: function showWindow(index) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	_knockout2.default.components.register('fend-sidebar', {
		template: template,
		viewModel: _Sidebar.Sidebar
	});
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.SidebarList = exports.Sidebar = undefined;
	...
	var Sidebar = exports.Sidebar = function () {
		function Sidebar() {
			...
			this.isVisible = _knockout2.default.observable(false);
		}
	...
	var SidebarList = exports.SidebarList = function () {
	   ...
	   */
			value: function itemClickHandler($index, $root, $sidebar) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	_knockout2.default.components.register('fend-sidebar-list', {
		viewModel: _Sidebar.SidebarList,
		template: template
	});
